...
void entity_init(ecs_entity_t e)
{
  // P("-- entity init --");
}
void entity_update(ecs_entity_t e, f32 dt)
{
  core_data_t* core_data = core_data_get();
  const f32 speed = 40.0f * dt;

  ECS_IMPORT(core_data->ecs_world, COMPONENTS);
  transform_comp_t* trans = ecs_get(core_data->ecs_world, e, transform_comp_t);
  
  TRANS_ROTATE_Y(trans, speed);  
  f32 x = (f32)sin(core_data->total_t);
  TRANS_MOVE_Y(trans, x * dt * 0.2f);  
