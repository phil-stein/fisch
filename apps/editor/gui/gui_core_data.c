...
void gui_core_data_win(ui_context* ctx, ui_rect win_rect, const u32 win_flags)
{
  ...
  if (nk_begin(ctx, "core_data", win_rect, win_flags)) 
  {
    ...
    if (nk_tree_push(ctx, NK_TREE_TAB, "play-state", NK_MINIMIZED))
    {
      ...
      // nk_bool phys_act    = core_data->phys_act;
      // nk_bool scripts_act = core_data->scripts_act;
      // nk_bool is_paused   = core_data->is_paused;
      nk_checkbox_label(ctx, "phys_act", (nk_bool*)(&core_data->phys_act) );
      nk_checkbox_label(ctx, "scripts_act", (nk_bool*)(&core_data->scripts_act) );
      nk_checkbox_label(ctx, "is_paused", (nk_bool*)(&core_data->is_paused) );
    ...
    {
      nk_checkbox_label(ctx, "use_async_asset_arrs", (nk_bool*)(&core_data->use_async_asset_arrs) );
    ...
    if (nk_tree_push(ctx, NK_TREE_TAB, "renderer", NK_MINIMIZED))
    {
      ...
      nk_checkbox_label(ctx, "show_shadows", (nk_bool*)(&core_data->show_shadows) );
      nk_tree_pop(ctx);
    }
