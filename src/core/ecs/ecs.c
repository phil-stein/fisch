...
// @NOTE: entity->init() get called in ecs_init(), in the editor they get called when play is pressed
...
// -- shared variables --
// shared variable dont use this just for error detection in ecs_entity_get(), extern def in state.h
bool __ecs_entity_get_error_shared = false;
int* __ecs_world_arr_len_ptr_shared = NULL;
...
// gest called in ecs_update() in editor
#ifndef EDITOR
  ecs_call_entity_init();
#endif
}
...
int ecs_entity_duplicate(entity_t* e, vec3 offset)
{
  ...
  if (e->point_light_idx >= 0)
  {
    ...
    ecs_point_light_add(p->offset, p->color, p->intensity, dupe);
  }

  return dupe;
  // return ecs_entity_add(pos, e->rot, e->scl, e->mesh, e->mat, e->init_f, e->update_f, e->table_idx);
}
void ecs_entity_remove_id(int id)
{
  ...
  {
    ecs_point_light_remove(world_arr[id].point_light_idx);
    // world_arr[id].point_light_idx = -1;
  }

  if (world_arr[id].parent > -1)
  {
    ecs_entity_remove_child_id(world_arr[id].parent, id, true);
  }
  // for (int i = 0; i < world_arr[id].children_len; ++i)
  while (world_arr[id].children_len > 0)  
  {
    ecs_entity_remove_child_id(id, world_arr[id].children[0], true);
  }
...
void ecs_entity_get_total_children_len(entity_t* e, u32* len)
{
  ...
  *len += e->children_len;
  ...
  {
    ecs_entity_get_total_children_len_id(e->children[i], len);
  }
}

// @NOTE: now inlined in state.h
// void ecs_entity_local_model(int id, mat4 out)
// void ecs_entity_local_model(entity_t* e, mat4 out)
// {  
...
void ecs_entity_update_global_model_dbg(entity_t* e, char* _file, int _line)
{
  ...
  {
    // same as ecs_entity_local_model()
    mat4_make_model(e->pos, e->rot, e->scl, e->model);
  }
  for (int i = 0; i < e->children_len; ++i)
  {
    // @NOTE: setting 'is_moved' updates children next 'ecs_update()'
    //        just updating doesn't work
    // ecs_entity_update_global_model(e->children[i]);
...
void ecs_entity_global_scale(int id, vec3 out)
{
  ...
  ecs_entity_global_scale(e->parent, out);
...
void ecs_point_light_remove(int id)
{
  ...
  {
    entity_t* e = ecs_entity_get(point_lights_arr[id].entity_id);
    e->point_light_idx = -1;
  }
