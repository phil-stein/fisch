...
int state_entity_add_from_template(vec3 pos, vec3 rot, vec3 scl, int template_idx, bool apply_template_offset)
{
  ...
  int id = state_entity_add(pos, rot, scl, mesh, mat, def->tags_flag, def->phys_flag, template_idx);

  if (def->pointlight.add)
  {
    state_point_light_add((f32*)def->pointlight.offset, (f32*)def->pointlight.color, (f32)def->pointlight.intensity, id);
  }
...
int state_entity_add(vec3 pos, vec3 rot, vec3 scl, int mesh, int mat, s64 tags_flags, entity_phys_flag phys_flag, int template_idx)
{
  TRACE();
...
int state_entity_add_empty(vec3 pos, vec3 rot, vec3 scl)
{
  ...
  return state_entity_add(pos, rot, scl, -1, -1, 0, 0, -1);
}
...
int state_entity_duplicate(entity_t* e, vec3 offset)
{
  ...
  int dupe = state_entity_add_from_template(pos, e->rot, e->scl, e->template_idx, false);
