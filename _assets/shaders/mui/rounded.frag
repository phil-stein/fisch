...
// 	uniform sampler2D tex;
  ...
  uniform float ratio_y; 
    ...
    vec2 coords_mapped = vec2( 
        // ( abs(coords.x * 2 -1) * ratio_x ), 
        // ( abs(coords.y * 2 -1) * ratio_y ));// * abs(1 - ratio));
        // ( abs(coords.x * 2 -1) - ( ratio_y * 1.0) ), 
        // ( abs(coords.y * 2 -1) - ( ratio_x * 1.0) ));// * abs(1 - ratio));
        ( abs(coords.x * 2 -1) - ( ratio_y * 0.15 ) ), 
        ( abs(coords.y * 2 -1) - ( ratio_x * 0.15 ) ));// * abs(1 - ratio));
